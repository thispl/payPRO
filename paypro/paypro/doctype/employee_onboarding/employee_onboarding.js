...
frappe.ui.form.on('Employee Onboarding', {
	...
	refresh: function(frm) {
		...
		if ((!frm.doc.employee) && (frm.doc.docstatus === 1)) {
			frm.add_custom_button(__('Employee'), function () {
				frappe.model.open_mapped_doc({
					method: "paypro.paypro.doctype.employee_onboarding.employee_onboarding.make_employee",
					frm: frm
				});
			}, __('Create'));
		...
		// 	frappe.call({
		// 		method: "paypro.paypro.utils.get_boarding_status",
		// 		args: {
	...
	employee_onboarding_template: function(frm) {
		...
		if (frm.doc.employee_onboarding_template) {
			frappe.call({
				method: "paypro.paypro.utils.get_onboarding_details",
				args: {
