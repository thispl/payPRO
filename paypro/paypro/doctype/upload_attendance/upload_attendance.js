...
paypro.paypro.AttendanceControlPanel = frappe.ui.form.Controller.extend({
	onload: function() {
	...
	get_template:function() {
		...
		window.location.href = repl(frappe.request.url +
			'?cmd=%(cmd)s&from_date=%(from_date)s&to_date=%(to_date)s', {
				cmd: "paypro.paypro.doctype.upload_attendance.upload_attendance.get_template",
	...
	show_upload() {
		...
		new frappe.ui.FileUploader({
			wrapper: $wrapper,
			method: 'paypro.paypro.doctype.upload_attendance.upload_attendance.upload'
		});
	},
...
cur_frm.cscript = new paypro.paypro.AttendanceControlPanel({frm: cur_frm});
