...
frappe.ui.form.on("Employee Attendance Tool", {
	...
	onload: function(frm) {
		...
		paypro.employee_attendance_tool.load_employees(frm);
	},
...
paypro.employee_attendance_tool = {
	load_employees: function(frm) {
		if(frm.doc.date) {
			frappe.call({
				method: "paypro.paypro.doctype.employee_attendance_tool.employee_attendance_tool.get_employees",
				args: {
					date: frm.doc.date,
				...
				callback: function(r) {
					if(r.message['unmarked'].length > 0) {
						...
						frm.EmployeeSelector = new paypro.EmployeeSelector(frm, frm.employee_area, r.message['unmarked'])
					}
					else{
					...
					if(r.message['marked'].length > 0) {
						...
						frm.marked_employee = new paypro.MarkedEmployee(frm, frm.marked_employee_area, r.message['marked'])
					}
					else{
...
paypro.MarkedEmployee = Class.extend({
...
paypro.EmployeeSelector = Class.extend({
	init: function(frm, wrapper, employee) {
		this.wrapper = wrapper;
		this.frm = frm;
		this.make(frm, employee);
	...
	make: function(frm, employee) {
		...
		mark_employee_toolbar.find(".btn-mark-present")
			...
			.on("click", function() {
				...
				frappe.call({
					method: "paypro.paypro.doctype.employee_attendance_tool.employee_attendance_tool.mark_employee_attendance",
					args:{
