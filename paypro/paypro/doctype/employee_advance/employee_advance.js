...
frappe.ui.form.on('Employee Advance', {
	...
	make_payment_entry: function(frm) {
		...
		if(frm.doc.__onload && frm.doc.__onload.make_payment_via_journal_entry) {
			method = "paypro.paypro.doctype.employee_advance.employee_advance.make_bank_entry"
		}
		return frappe.call({
			method: method,
			args: {
	...
	make_expense_claim: function(frm) {
		return frappe.call({
			method: "paypro.paypro.doctype.expense_claim.expense_claim.get_expense_claim",
			args: {
	...
	make_return_entry: function(frm) {
		frappe.call({
			method: 'paypro.paypro.doctype.employee_advance.employee_advance.make_return_entry',
			args: {
	...
	employee: function (frm) {
		if (frm.doc.employee) {
			return frappe.call({
				method: "paypro.paypro.doctype.employee_advance.employee_advance.get_due_advance_amount",
				args: {
					"employee": frm.doc.employee,
					"posting_date": frm.doc.posting_date
				},
